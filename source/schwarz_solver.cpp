...
SolverBase<ValueType, IndexType>::SolverBase(
    ...
    : Initialize<ValueType, IndexType>(settings, metadata), settings(settings),
      metadata(metadata) {
  auto local_rank =
  ...
  comm_struct.get_displacements = std::shared_ptr<vec_itype>(
      new vec_itype(settings.executor->get_master(), temp.begin(), temp.end()),
      std::default_delete<vec_itype>());
...
void SolverBase<ValueType, IndexType>::initialize() {
  ...
  if (settings.explicit_laplacian) {
    ...
    Initialize<ValueType, IndexType>::setup_global_matrix(matrix,
                                                          this->global_matrix);
#endif
  }
  ...
  if (settings.print_matrices && settings.executor_string != "cuda") {
    Utils<ValueType, IndexType>::print_matrix(this->local_matrix.get(),
                                              metadata.my_rank, "local_mat");
...
void SolverBase<ValueType, IndexType>::run(
  ...
  for (; metadata.iter_count < metadata.max_iters; ++(metadata.iter_count)) {
    ...
    if (num_converged_procs == metadata.num_subdomains) {
      ...
      MEASURE_ELAPSED_FUNC_TIME((Solve<ValueType, IndexType>::local_solve(
                                    settings, metadata, this->triangular_factor,
                                    temp_loc_solution, this->local_solution)),
                                3, metadata.my_rank, local_solve,
                                metadata.iter_count);
  ...
  Solve<ValueType, IndexType>::compute_residual_norm(
      settings, metadata, global_matrix, global_rhs, solution_vector, mat_norm,
      rhs_norm, sol_norm, residual_norm);
  ...
  if (metadata.my_rank == 0) {
    ...
    // std::shared_ptr<vec_vtype> cpu_solution_vector = vec_vtype::create(
    //     settings.executor->get_master(), gko::dim<2>(metadata.global_size, 1));
    // cpu_solution_vector->copy_from(solution_vector.get());
    // std::copy(cpu_solution_vector->get_values(),
    //           cpu_solution_vector->get_values() + metadata.global_size,
    //           solution.begin());
  }
}
...
void SolverRAS<ValueType, IndexType>::setup_local_matrices(
  ...
  if (partition_settings == Settings::partition_settings::partition_metis) {
    auto gmat_temp =
        mtx::create(settings.executor->get_master(), global_matrix->get_size(),
    ...
    for (auto row = 0; row < metadata.global_size; ++row) {
      for (auto col = gmat_row_ptrs[permutation[row]];
        ...
        gmat_temp->get_col_idxs()[nnz] = i_permutation[gmat_col_idxs[col]];
  ...
  if (nnz_interface > 0) {
    interface_matrix_compute =
        mtx::create(settings.executor->get_master(), gko::dim<2>(local_size_x),
                    nnz_interface);
  } else {
  ...
  for (auto i = first_row[my_rank]; i < first_row[my_rank + 1]; ++i) {
    for (auto j = gmat_row_ptrs[i]; j < gmat_row_ptrs[i + 1]; ++j) {
      if (global_to_local[gmat_col_idxs[j]] != 0) {
        lmat_col_idxs[nnz_local] = global_to_local[gmat_col_idxs[j]] - 1;
    ...
    if (p != my_rank) {
      if (recv[p] != 0) {
        MPI_Isend((global_get[pp]), 1, mpi_itype, p, 1, MPI_COMM_WORLD,
                  &send_req1[p]);
        MPI_Isend((global_get[pp]), 1 + (global_get[pp])[0], mpi_itype, p, 2,
                  MPI_COMM_WORLD, &send_req2[p]);
        pp++;
      } else {
        MPI_Isend(&zero, 1, mpi_itype, p, 1, MPI_COMM_WORLD, &send_req1[p]);
      }
      MPI_Irecv(&send[p], 1, mpi_itype, p, 1, MPI_COMM_WORLD, &recv_req1[p]);
    }
  }
...
void SolverRAS<ValueType, IndexType>::setup_windows(
  ...
  MPI_Alltoall(get_displacements, 1, mpi_itype, put_displacements, 1, mpi_itype,
               MPI_COMM_WORLD);
  ...
  if (settings.comm_settings.enable_onesided) {
    ...
    MPI_Win_create(this->convergence_vector->get_data(),
                   ...
                   MPI_INFO_NULL, MPI_COMM_WORLD, &(this->window_convergence));
  } else {
    // Twosided
    // residual_vector = vec_vtype::create(settings.executor,
    // gko::dim<2>(num_subdomains, 1));
    ...
    if (settings.comm_settings.enable_push_one_by_one) {
      for (auto p = 0; p < num_neighbors_out; p++) {
        if ((global_put[p])[0] > 0) {
          ...
          for (auto i = 1; i <= (global_put[p])[0]; i++) {
            MPI_Put(&local_solution->get_values()[(local_put[p])[i]], 1,
          ...
          if (settings.comm_settings.enable_flush_all) {
            ...
            MPI_Win_flush_local(neighbors_out[p], comm_struct.window_x);
          }
        }
      }
    } else {
      ...
      for (auto p = 0; p < num_neighbors_out; p++) {
        ...
        if ((global_put[p])[0] > 0) {
          ...
          MPI_Put(&send_buffer[num_put], (global_put[p])[0], mpi_vtype,
                  neighbors_out[p], put_displacements[neighbors_out[p]],
                  (global_put[p])[0], mpi_vtype, comm_struct.window_buffer);
          if (settings.comm_settings.enable_flush_all) {
            MPI_Win_flush(neighbors_out[p], comm_struct.window_buffer);
          } else if (settings.comm_settings.enable_flush_local) {
            MPI_Win_flush_local(neighbors_out[p], comm_struct.window_buffer);
          }
      ...
      if ((global_put[p])[0] > 0) {
        ...
        for (auto i = 1; i <= (global_get[p])[0]; i++) {
          MPI_Get(&local_solution->get_values()[(local_get[p])[i]], 1,
                  mpi_vtype, neighbors_in[p], (remote_get[p])[i], 1, mpi_vtype,
                  comm_struct.window_x);
        }
        if (settings.comm_settings.enable_flush_all) {
...
void exchange_boundary_twosided(
  ...
  for (auto p = 0; p < num_neighbors_out; p++) {
    // send
    if ((global_put[p])[0] > 0) {
      if (settings.comm_settings.enable_overlap && metadata.iter_count > 1) {
...
void SolverRAS<ValueType, IndexType>::update_boundary(
  ...
  if (metadata.num_subdomains > 1 && settings.overlap > 0) {
    auto temp_solution =
        vec_vtype::create(settings.executor, local_solution->get_size(),
