...
template <typename ValueType, typename IndexType> class Initialize;
...
void Solve<ValueType, IndexType>::setup_local_solver(
  ...
  //          "local solver schemes."));

  // metadata.iter_count   = 0;
  local_residual_vector = vec::create(settings.executor->get_master(),
                                      gko::dim<2>(metadata.max_iters + 1, 1));
  local_residual_vector_out = vec::create(
      settings.executor->get_master(), gko::dim<2>(metadata.max_iters + 1, 1));
  ...
  if ((solver_settings ==
    ...
    if (metadata.my_rank == 0)
      std::cout << " Local direct factorization with CHOLMOD " << std::endl;
    ...
    cholmod.system_matrix = cholmod_allocate_sparse(
        num_rows, num_rows, num_nonzeros, sorted, packed, stype, CHOLMOD_REAL,
        &(cholmod.settings));
    ...
    IndexType *col_ptrs = static_cast<IndexType *>(cholmod.system_matrix->p);
    IndexType *row_idxs = static_cast<IndexType *>(cholmod.system_matrix->i);
    ValueType *cmat_values = static_cast<ValueType *>(cholmod.system_matrix->x);
    ...
    cholmod_factorize(cholmod.system_matrix, cholmod.L_factor,
                      &(cholmod.settings));
    auto factor_nnz = (static_cast<IndexType *>(cholmod.L_factor->p))[num_rows];
    ...
    if (solver_settings ==
      ...
      // triangular_factor = mtx::create(settings.executor,
      //                                 temp_triangular_factor->get_size());
      // triangular_factor->copy_from(gko::lend(temp_triangular_factor));
      // triangular_factor->sort_by_column_index();
      // auto trfac_trans =
      //   static_cast<std::unique_ptr<mtx>>(triangular_factor->transpose());
      // trans_triangular_factor = triangular_factor->transpose()
      if (metadata.my_rank == 0) {
      ...
      this->U_solver =
          u_trs::build().on(settings.executor)->generate((triangular_factor));
...
bool Solve<ValueType, IndexType>::check_local_convergence(
  ...
  if (tolerance >= 0.0) {
    ...
    if (local_resnorm0 < 0.0)
      local_resnorm0 = local_resnorm;
...
void Solve<ValueType, IndexType>::check_global_convergence(
  ...
  if (settings.convergence_settings.enable_global_check) {
    ...
    // > ** locally ** check for the global convergence
    if (global_resnorm >= 0.0) {
      if ((global_resnorm0) < 0.0)
        global_resnorm0 = global_resnorm;
      // TODO: Check if tol*tol is necessary ?
      if ((global_resnorm) / (global_resnorm0) <= tolerance)
        (converged_all_local)++;
    }
  } else {
    // --------------------------------------------------- //
    // convergence detection based on local residual norm //
    // --------------------------------------------------- //
    // TODO: Check if tol*tol is necessary ?
  ...
  if (settings.comm_settings.enable_onesided) {
    if (settings.convergence_settings.enable_global_simple_tree) {
      ...
      ConvergenceTools::global_convergence_check_onesided_propagate(
          settings, metadata, comm_struct, convergence_vector, convergence_sent,
          convergence_local, converged_all_local, num_converged_procs,
          window_convergence);
    }
  } else { // two-sided MPI
    if (settings.convergence_settings.enable_global_check) {
      ...
      MPI_Allreduce(&local_converged, &num_converged_procs, 1, MPI_INT, MPI_SUM,
                    MPI_COMM_WORLD);
    }
  }
}
...
void Solve<ValueType, IndexType>::update_residual(
  ...
  local_matrix->apply(neg_one.get(), gko::lend(global_old_solution), one.get(),
...
void Solve<ValueType, IndexType>::compute_residual_norm(
  ...
  // for (auto i = 0; i < metadata.local_size; ++i)
  //   {
  //     local_sol->get_values()[first_row + i] =
  //       solution_vector->get_values()[first_row + i];
  //   }
