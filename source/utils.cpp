...
namespace SchwarzWrappers
{
  ...
  int Utils<ValueType, IndexType>::get_local_rank(MPI_Comm mpi_communicator)
  {
    ...
    int      rank;
    MPI_Comm_split_type(
      mpi_communicator, MPI_COMM_TYPE_SHARED, 0, MPI_INFO_NULL, &local_comm);
    MPI_Comm_rank(local_comm, &rank);
    return rank;
  }

  template <typename ValueType, typename IndexType>
  int Utils<ValueType, IndexType>::get_local_num_procs(
  ...
  void Utils<ValueType, IndexType>::print_matrix(
    const gko::matrix::Csr<ValueType, IndexType> *matrix,
    int                                           rank,
    std::string                                   name)
  {
    std::ofstream file;
    ...
    for (auto row = 0; row < matrix->get_size()[0]; row++)
      {
        for (auto col = matrix->get_const_row_ptrs()[row];
             col < matrix->get_const_row_ptrs()[row + 1];
             col++)
          {
