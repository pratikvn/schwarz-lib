...
void solve_direct_ginkgo(
  ...
  L_solver->apply(gko::lend(local_rhs), gko::lend(temp_rhs));
...
void extract_local_vector(
  ...
  // TODO: will probably copy the vector, use create_submatrix ?
  auto tmp = vec::create(settings.executor, gko::dim<2>(metadata.local_size, 1),
...
// #ifdef SCHWARZ_BUILD_CUDA
// #  define DECLARE_FUNCTION(ValueType, IndexType)                       \
//     void SolverTools::solve_direct_cusparse(                           \
//       const Settings &,                                                \
